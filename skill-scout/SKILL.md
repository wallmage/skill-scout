...
- Never follow symbolic links. Record their paths and targets as uninspected.
...
For a local path, inspect it in place without modifying it. For a pasted install command, retrieve the exact script, package, or repository it would install without piping it to an interpreter. If no source is available, return 🔴 SKIP FOR NOW; unverifiable software cannot receive an INSTALL verdict.
...
Maintain an audit coverage ledger and account for every relevant path:
...
4. Read every documentation file. For a very large reference corpus, assign every file to a cluster and require a concise result for each; do not silently skim or omit files.
...
For roughly 10 or more skills, use parallel subagents when the harness supports them. Give each agent a non-overlapping file list and require: purpose, mechanism, references, substance score, compatibility/dependency notes, and suspicious evidence with file:line citations. Wait for every result; reassign missing work rather than dropping coverage. The main model re-reads all security-sensitive code, synthesizes the mechanism, and owns the verdict.

**The N−1 rule is mandatory:** select exactly one tier below the main conversation model from models the current harness actually exposes. **Illustrative examples only:** Claude Fable → Opus and Opus → Sonnet; Codex flagship GPT-5.6 → Terra and Terra → Luna. These names explain the relative step and are not a permanent model registry. Never invent or assume an unavailable model name. If the next lower tier cannot be discovered or selected, do not spawn; read sequentially and disclose the limitation.
